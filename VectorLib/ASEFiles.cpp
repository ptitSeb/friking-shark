...
void CASEFileType::ToGCM(CGCMFileType *pFile)
{
	...
	for (int f=0;f<m_Scene.nFrameCount;f++)
	{
		...
		for (int m=-1;m<(int)m_vMaterials.size();m++)
		{
			...
			// Despues se realocaran estos buffers a su tama�o correcto
			...
			// Se reducen los buffers de los vertices a su tama�o correcto
			// han sido alocados a su tama�o maximo por comodidad
			...
			// Si hay vertices para el material se a�ade el render buffer 
