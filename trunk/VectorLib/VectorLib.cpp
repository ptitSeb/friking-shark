...
int CBSPNode::GetContent(const CVector &position)
{
	...
	if(content!=CONTENT_EMPTY){return content;}
	return pChild[1]->GetContent(position);
	*/
/*
...
void CalcCameraVolume(CVector vPosition,CVector vAngles, double dViewAngle, double dAspectRatio, double dNear, double dFar,CVector *pPoints)
{
	...
	vNearOffset.c[0]=dNear*tan(DegreesToRadians((dViewAngle*dAspectRatio)/2.0));
	vNearOffset.c[2]=dNear*tan(DegreesToRadians(dViewAngle)/2.0);
	vFarOffset.c[0]=dFar*tan(DegreesToRadians((dViewAngle*dAspectRatio)/2.0));
	vFarOffset.c[2]=dFar*tan(DegreesToRadians(dViewAngle)/2.0);
