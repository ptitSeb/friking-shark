...
ISystemPersistencyNode *CConfigFileNode::AddNode(std::string &id){return GetAddNode_Internal(id.c_str(),true);}
ISystemPersistencyNode *CConfigFileNode::GetNode(std::string &id){return GetAddNode_Internal(id.c_str(),false);}
ISystemPersistencyNode *CConfigFileNode::AddNode(const char *id){return GetAddNode_Internal(id,true);}
ISystemPersistencyNode *CConfigFileNode::GetNode(const char *id){return GetAddNode_Internal(id,false);}
unsigned long			CConfigFileNode::GetNodeCount(){return (unsigned long)m_mNodes.size();}

ISystemPersistencyNode *CConfigFileNode::GetNode(unsigned long nIndex)
{
	if(nIndex>=m_vNodes.size()){return NULL;}
	return m_vNodes[nIndex];
}

void CConfigFileNode::DeleteNode(std::string &sNodePath){DeleteNode(sNodePath.c_str());}
void CConfigFileNode::DeleteNode(const char *pNodePath)
{
    ...
    else
    {
        ...
        if(i!=m_mNodes.end())
        {
			...
			{
				if(*iVector==pNode){m_vNodes.erase(iVector);break;}
			}
            delete pNode;
        }
    }
...
const char *CConfigFileNode::GetName(){return m_sName.c_str();}
const char *CConfigFileNode::GetValue(){return m_bHasValue?m_sValue.c_str():NULL;}
...
char *CConfigFile::RemoveSpacesAndTabs(char *pString)
{
	...
	while(nLength && (pString[nLength-1]==' ' || pString[nLength-1]=='\t')){nLength--;}
	pString[nLength]=0;
...
bool CConfigFile::Open(std::string &sFileName){return Open(sFileName.c_str());}
bool CConfigFile::Open(const char *pFileName)
{
    ...
    m_hFile=CreateFile(pFileName,GENERIC_READ,FILE_SHARE_READ,NULL,OPEN_EXISTING,FILE_ATTRIBUTE_NORMAL,NULL);
	...
	*pNormalizationResult=0;
    ...
    while(pToken=strtok(pToken?NULL:pNormalizedBuffer,"\n"))
    {
        ...
        else // Node or value
        {
			...
			if(pEqual){*pEqual=0;pValue=pEqual+1;}
			if(pName){pName=RemoveSpacesAndTabs(pName);}
			...
			if(pValue)
			{
				...
				if(pValue && pValue[0]=='"')
				{
					...
					if(nValueLen && pValue[nValueLen-1]=='"'){pValue[nValueLen-1]=0;}
	...
	if(pNormalizedBuffer){delete [] pNormalizedBuffer;pNormalizedBuffer=NULL;}
	CloseHandle(m_hFile);
    m_hFile=INVALID_HANDLE_VALUE;
    m_pBuffer=NULL;
    return true;
...
bool CConfigFile::Save(std::string &sFileName){return Save(sFileName.c_str());}
bool CConfigFile::Save(const char *pFileName)
{
    m_hFile=CreateFile(pFileName,GENERIC_WRITE,0,NULL,CREATE_ALWAYS,FILE_ATTRIBUTE_NORMAL,NULL);
...
void CConfigFile::SaveLine(const char *psValue)
{
	...
	sTempLine+=psValue;
	sTempLine+="\r\n";
    WriteFile(m_hFile,sTempLine.c_str(),(DWORD)sTempLine.length(),&dwWritten,NULL);
}
...
ISystemPersistencyNode *CConfigFile::GetRoot(){return &m_RootNode;}
ISystemPersistencyNode *CConfigFile::GetNode(std::string &id){return m_RootNode.GetNode(id);}
ISystemPersistencyNode *CConfigFile::AddNode(std::string &id){return m_RootNode.AddNode(id);}
ISystemPersistencyNode *CConfigFile::GetNode(const char *id){return m_RootNode.GetNode(id);}
ISystemPersistencyNode *CConfigFile::AddNode(const char *id){return m_RootNode.AddNode(id);}
