...
/*
bool CWorldManager::UpdateWater()
{
	*ppWaterFrame=NULL;
	if(!pInfo->bEnabled){return false;}

	CMaterial *pMaterial=new CMaterial;
	pMaterial->pTexture=pStore->LoadTexture(pInfo->sTextureFile);
	pMaterial->nTextureIndex=pMaterial->pTexture?pMaterial->pTexture->m_nOpenGlIndex:-1;
	pMaterial->fOpacity=pInfo->dOpacity;
	if(pMaterial->pTexture==NULL)
	{
		delete pMaterial;
		return false;
	}
	pStore->AddMaterial(pMaterial);

	CFrame *pFrame=new CFrame;
	CVector vMins,vMaxs;
	double dAbsWaterHeight=(pInputFrame->m_vMaxs.c[1]-pInputFrame->m_vMins.c[1])*pInfo->dHeight+pInputFrame->m_vMins.c[1];
	bool bFirstVertex=true;

	for(int nBuffer=0;nBuffer<pInputFrame->m_nPreprocessedRenderBuffers;nBuffer++)
	{
		CPreprocessedRenderBuffer *pBuffer=&pInputFrame->m_pPreprocessedRenderBuffers[nBuffer];
		GLuint *pFaceCursor=pBuffer->pTriangleVertexIndexes;

		for(int nTriangle=0;nTriangle<pBuffer->nTriangles;nTriangle++)
		{
			// Lectura de la informacion de la cara
			CVector vVertexes[3];
			for(unsigned long nVertex=0;nVertex<3;nVertex++,pFaceCursor++)
			{
				GLuint nVertexIndex=*pFaceCursor;
				GLfloat *pVertexCursor=&pBuffer->pVertexArray[nVertexIndex*3];
				for(unsigned long nCoord=0;nCoord<3;nCoord++)
				{
					vVertexes[nVertex].c[nCoord]=*pVertexCursor++;
				}
			...
			if(bInWater)
			{
				...
				for(unsigned long nVertex=0;nVertex<3;nVertex++)
				{
					if(bFirstVertex || vVertexes[nVertex].c[0]<vMins.c[0]){vMins.c[0]=vVertexes[nVertex].c[0];}
					if(bFirstVertex || vVertexes[nVertex].c[0]>vMaxs.c[0]){vMaxs.c[0]=vVertexes[nVertex].c[0];}
					if(bFirstVertex || vVertexes[nVertex].c[2]<vMins.c[2]){vMins.c[2]=vVertexes[nVertex].c[2];}
					if(bFirstVertex || vVertexes[nVertex].c[2]>vMaxs.c[2]){vMaxs.c[2]=vVertexes[nVertex].c[2];}
	...
	CMaterialPolygon polygons[2];
	CVector waterVertexes[4];
	CVector waterTexVertexes[4];

	waterVertexes[0].c[0]=vMins.c[0];
	waterVertexes[0].c[1]=dAbsWaterHeight;
	waterVertexes[0].c[2]=vMins.c[2];

	waterVertexes[1].c[0]=vMins.c[0];
	waterVertexes[1].c[1]=dAbsWaterHeight;
	waterVertexes[1].c[2]=vMaxs.c[2];

	waterVertexes[2].c[0]=vMaxs.c[0];
	waterVertexes[2].c[1]=dAbsWaterHeight;
	waterVertexes[2].c[2]=vMaxs.c[2];

	waterVertexes[3].c[0]=vMaxs.c[0];
	waterVertexes[3].c[1]=dAbsWaterHeight;
	waterVertexes[3].c[2]=vMins.c[2];

	waterTexVertexes[0].c[0]=0;
	waterTexVertexes[0].c[1]=0;

	waterTexVertexes[1].c[0]=0;
	waterTexVertexes[1].c[1]=pInfo->dVerticalResolution;

	waterTexVertexes[2].c[0]=pInfo->dHorizontalResolution;
	waterTexVertexes[2].c[1]=pInfo->dVerticalResolution;

	waterTexVertexes[3].c[0]=pInfo->dHorizontalResolution;
	waterTexVertexes[3].c[1]=0;

	for(int x=0;x<2;x++)
	{
		polygons[x].m_nVertexes=3;
		polygons[x].m_pVertexes=new CVector[3];
		polygons[x].m_pTextureCoords=new CVector[3];
		polygons[x].m_pVertexNormals=new CVector[3];
		polygons[x].m_pMaterial=pMaterial;

		for(int y=0;y<3;y++)
		{
			polygons[x].m_pVertexNormals[y]=CVector(0,1,0);
		}
	}

	polygons[0].m_pVertexes[0]=waterVertexes[0];
	polygons[0].m_pVertexes[1]=waterVertexes[1];
	polygons[0].m_pVertexes[2]=waterVertexes[2];
	polygons[0].m_pTextureCoords[0]=waterTexVertexes[0];
	polygons[0].m_pTextureCoords[1]=waterTexVertexes[1];
	polygons[0].m_pTextureCoords[2]=waterTexVertexes[2];

	polygons[1].m_pVertexes[0]=waterVertexes[0];
	polygons[1].m_pVertexes[1]=waterVertexes[2];
	polygons[1].m_pVertexes[2]=waterVertexes[3];
	polygons[1].m_pTextureCoords[0]=waterTexVertexes[0];
	polygons[1].m_pTextureCoords[1]=waterTexVertexes[2];
	polygons[1].m_pTextureCoords[2]=waterTexVertexes[3];


	pFrame->m_nPolygons=2;
	pFrame->m_pPolygons=polygons;
	pFrame->GeneratePreprocessedRenderBuffer();
	pFrame->m_pPreprocessedRenderBuffers[0].pTex1VertexArray=new GLfloat[pFrame->m_pPreprocessedRenderBuffers[0].nVertexes*2];
	memcpy(pFrame->m_pPreprocessedRenderBuffers[0].pTex1VertexArray,pFrame->m_pPreprocessedRenderBuffers[0].pTexVertexArray,pFrame->m_pPreprocessedRenderBuffers[0].nVertexes*2*sizeof(GLfloat));
	pFrame->GenerateBufferObjects();
	pFrame->m_nPolygons=0;
	pFrame->m_pPolygons=NULL;
	*ppWaterFrame=pFrame;
	return true;
}
*/
...
void CWorldManager::SetupRenderingEnvironment( IGenericRender *piRender )
{
	...
	if(m_TerrainModel.m_piModel)
	{
		...
		piRender->SetSunLight(vSunPos,vForward,m_TerrainSun.vColor);
	}
