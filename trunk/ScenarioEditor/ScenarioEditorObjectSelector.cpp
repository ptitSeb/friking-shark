...
void CScenarioEditorObjectSelector::OnDraw(IGenericRender *piRender)
{
	if(m_piSTObjectList)
	{
		SGameRect sListRect;
...
void CScenarioEditorObjectSelector::OnInitDialog()
{
	...
	if(m_piSTObjectList)
	{
		for(unsigned long x=0;x<m_pvObjects->size();x++)
		{
			...
			label.Create(m_piSystem,"CScenarioEditorObjectLabel","");
			if(button.m_piButton)
			{
...
void CScenarioEditorObjectSelector::OnEndDialog()
{
	for(unsigned long x=0;x<m_vButtons.size();x++)
	{
...
bool CScenarioEditorObjectSelector::SelectObject(IGameWindow *piParent,std::vector<IDesignObject *> *vObjects,unsigned long *pnSelectedObject,double dButtonSizeX,double dButtonSizeY)
{
...
void CScenarioEditorObjectSelector::OnButtonClicked(IGameGUIButton *piControl)
{
	for(unsigned long x=0;x<m_vButtons.size();x++)
	{
