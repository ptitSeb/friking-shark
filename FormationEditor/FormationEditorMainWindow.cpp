...
bool CFormationEditorMainWindow::InitWindow(IGameWindow *piParent,bool bPopup)
{
	...
	if(m_GameControllerWrapper.m_piGameController)
	{
	  ...
	  m_Formation.Create("FormationTypes","CFormationType","__unnamed__");
	}
...
void CFormationEditorMainWindow::OnDraw(IGenericRender *piRender)
{
	...
	else
	{
		...
		for(unsigned int x=0;x<m_vEntityControls.size();x++)
		{
			...
			m_Formation.m_piFormationTypeDesign->GetElementRoutePoint(x,0,&point);
			m_Formation.m_piFormationTypeDesign->GetElementRoutePoint(x,1,&point2);
...
void CFormationEditorMainWindow::ProcessFileOpen()
{
	...
	{
		Reset();
		if(m_Formation.Attach(vFormationTypes[nSelectedFormationType]))
		{
			UpdateEntityControls();
			UpdateLayerPanel();
			UpdateCaption();
...
void CFormationEditorMainWindow::OnButtonClicked(IGameGUIButton *piControl)
{
	...
	if(m_piBTNewEntity==piControl)
	{
		...
		if(m_ObjectSelector.m_piObjectSelector->SelectObject(this,&vEntityTypes,&nSelectedEntityType))
		{
			...
			{
				SRoutePoint point;
				unsigned long nObjectIndex=m_Formation.m_piFormationTypeDesign->AddElement();
				GetAirPlaneCoordinatesFromCursorPos(m_rRealRect.w/2,m_rRealRect.h/2,&point.vPosition);
				m_Formation.m_piFormationTypeDesign->AddElementRoutePoint(nObjectIndex,0,point);
				m_Formation.m_piFormationTypeDesign->SetElementEntityType(nObjectIndex,piEntityType);
			}
			REL(piObject);
			REL(piEntityType);
			UpdateEntityControls();
	...
	if(pEntity)
	{
		...
		{
			m_Formation.m_piFormationTypeDesign->RemoveElement(m_nSelectedEntity);
			UpdateEntityControls();
		}
		else if(piControl==m_piBTEntitySample)
		{
			...
			{
				IEntityType *piEntityType=QI(IEntityType,vEntityTypes[nSelectedEntityType]);
				m_Formation.m_piFormationTypeDesign->SetElementEntityType(m_nSelectedEntity,piEntityType);
				REL(piEntityType);
		...
		{
			m_Formation.m_piFormationTypeDesign->ClearElementRoute(m_nSelectedEntity);
			m_nSelectedRoutePoint=-1;
		}
	}
...
void CFormationEditorMainWindow::UpdateCaption()
{
	if(m_Viewport.m_piViewport)
	{
		...
		if(m_Formation.m_piObject){sCaption+=" - "+m_Formation.m_piObject->GetName();}
...
void CFormationEditorMainWindow::CenterCamera()
{
	...
	if(m_nSelectedEntity!=-1 && m_vEntityControls[m_nSelectedEntity]->m_piDesignObject)
	{
		...
		m_Formation.m_piFormationTypeDesign->GetElementRoutePoint(m_nSelectedEntity,0,&point);
		bCenter=true;
	...
	else if(nKey==GK_DELETE)
	{
		*pbProcessed=true;
		if(m_nSelectedEntity!=-1 && m_nSelectedRoutePoint!=-1 && 
			m_Formation.m_piFormationTypeDesign->GetElementRoutePoints(m_nSelectedEntity)>1)
		{
			m_Formation.m_piFormationTypeDesign->RemoveElementRoutePoint(m_nSelectedEntity,m_nSelectedRoutePoint);
			if(m_nSelectedRoutePoint>=(int)m_Formation.m_piFormationTypeDesign->GetElementRoutePoints(m_nSelectedEntity))
			{
				m_nSelectedRoutePoint=m_Formation.m_piFormationTypeDesign->GetElementRoutePoints(m_nSelectedEntity)-1;
			}
		}
	}
	else if(nKey==GK_INSERT)
	{
		*pbProcessed=true;
		if(m_nSelectedEntity!=-1)
		{
			...
			m_Formation.m_piFormationTypeDesign->GetElementRoutePoint(m_nSelectedEntity,0,&firstPoint);
			...
			else
			{
				...
				{
					m_Formation.m_piFormationTypeDesign->GetElementRoutePoint(m_nSelectedEntity,m_nSelectedRoutePoint-1,&sOldPoint1);
				}
				else
				{
					sOldPoint1.vPosition=firstPoint.vPosition;
				}
				m_Formation.m_piFormationTypeDesign->GetElementRoutePoint(m_nSelectedEntity,m_nSelectedRoutePoint,&sOldPoint2);
				CVector vDirection=sOldPoint2.vPosition-sOldPoint1.vPosition;
				vDirection.N();
				sRoutePoint.vPosition+=sOldPoint2.vPosition+vDirection*(0.1);
			}
			m_Formation.m_piFormationTypeDesign->AddElementRoutePoint(m_nSelectedEntity,m_nSelectedRoutePoint+1,sRoutePoint);
...
void CFormationEditorMainWindow::OnMouseDown( int nButton,double dx,double dy )
{
	...
	{	
		...
		for(unsigned int x=0;x<m_vEntityControls.size();x++)
		{
			...
			for(unsigned y=1;y<m_Formation.m_piFormationTypeDesign->GetElementRoutePoints(x);y++)
			{
				SRoutePoint point;
				m_Formation.m_piFormationTypeDesign->GetElementRoutePoint(x,y,&point);
				m_Render.m_piRender->SetSelectionId(y);
				CVector vWorld=FormationToWorld(point.vPosition);
				m_Render.m_piRender->RenderPoint(vWorld,(int)x==m_nSelectedRoutePoint?10:5,CVector(0.8,0.8,0.8),1.0);
			}
			...
			if(nNewSelection!=-1 && m_nSelectedEntity==(int)x && m_nSelectedRoutePoint==nNewSelection)
			{
				if(DetectDrag(dx,dy))
				{
					...
					SRoutePoint point;
					m_Formation.m_piFormationTypeDesign->GetElementRoutePoint(x,nNewSelection,&point);
...
void CFormationEditorMainWindow::OnMouseMove( double x,double y )
{
	if(m_bMovingRoutePoint && m_nSelectedRoutePoint!=-1)
	{
		...
		if(GetAirPlaneCoordinatesFromCursorPos(x,y,&vTemp))
		{
			...
			m_Formation.m_piFormationTypeDesign->SetElementRoutePoint(m_nSelectedEntity,m_nSelectedRoutePoint,point);
		}
	}
}
...
void CFormationEditorMainWindow::UpdateEntityControls()
{
	...
	if(m_Formation.m_piFormationTypeDesign)
	{
		for(x=0;x<m_Formation.m_piFormationTypeDesign->GetElements();x++)
		{
			IEntityType		 *piEntityType=NULL;
			m_Formation.m_piFormationTypeDesign->GetElementEntityType(x,&piEntityType);
...
void CFormationEditorMainWindow::RenderRoute( IGenericRender * piRender, int nEntity,int nSelectedEntity, int nSelectedRoutePoint )
{
	...
	SRoutePoint point;
	m_Formation.m_piFormationTypeDesign->GetElementRoutePoint(nEntity,0,&point);
	CVector vPreviousPoint=FormationToWorld(point.vPosition);
	for(unsigned x=1;x<m_Formation.m_piFormationTypeDesign->GetElementRoutePoints(nEntity);x++)
	{
		double dPointSize=5;
		...
		m_Formation.m_piFormationTypeDesign->GetElementRoutePoint(nEntity,x,&point);
		CVector vWorld=FormationToWorld(point.vPosition);
